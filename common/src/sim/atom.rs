...
/// connection. It also stores the `src_loc` for reference back to the original canvas.
///
/// Atoms are unique within the Canvas from which they were generated. That is to say, all the
/// different `Path` instances derived from a single IO cell in a canvas will have unique
/// AtomType::TerminalIoPin directions.
///
/// Atoms are geared toward simulation and don't map 1:1 with the canvas drawing. For example, a
/// MOSFET cell in the canvas will always correspond to exactly 4 Atoms in 3 different Paths:
